...
BASE_URL = os.getenv("BASE_URL", "http://localhost:8415")
...
# Seed de dados de DEMO (barbearias/agendamentos fictícios). Por padrão segue
# o ambiente (liga em dev, desliga em produção). Defina SEED_DEMO=true para
# popular também a produção (deploy educacional) ou =false para desligar em dev.
# O seed é idempotente (só roda se a tabela de barbearia estiver vazia).
_SEED_DEMO_ENV = os.getenv("SEED_DEMO")
SEED_DEMO = (
    IS_DEVELOPMENT
    if _SEED_DEMO_ENV is None
    else _SEED_DEMO_ENV.strip().lower() == "true"
)
