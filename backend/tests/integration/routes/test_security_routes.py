...
class TestEscalacaoPrivilegios:
    ...
    def test_vendedor_nao_acessa_admin_usuarios_retorna_403(self, vendedor_autenticado):
        """Vendedor (perfil não-admin) não deve acessar área admin (admin-only)"""
        response = vendedor_autenticado.get("/api/admin/usuarios")
        assert response.status_code == status.HTTP_403_FORBIDDEN
