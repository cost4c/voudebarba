...
class TestListar:
    ...
    def test_filtro_por_perfil(self, admin_autenticado, criar_usuario_direto):
        ...
        resp = admin_autenticado.get(
            f"/api/admin/usuarios/?perfil={Perfil.BARBEARIA.value}"
        )
        assert resp.status_code == status.HTTP_200_OK
        corpo = resp.json()
        assert corpo["total"] == 2
        assert all(i["perfil"] == Perfil.BARBEARIA.value for i in corpo["items"])
...
class TestAlterar:
    ...
    def test_alterar_sucesso_200(self, admin_autenticado, criar_usuario_direto):
        ...
        assert corpo["perfil"] == Perfil.BARBEARIA.value
